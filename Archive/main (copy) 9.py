...
def list_chat_users_with_status(chat_id: int, current_user_email: str = Depends(get_current_user)):
    try:
        ...
        # Fetch all users with the same corporate code
        cursor.execute("SELECT id, name, email, mobile_number FROM users WHERE corporate_code = %s AND id != %s", (corporate_code, user_id))
