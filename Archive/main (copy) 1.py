...
def create_chat(chat_participant_id: Optional[int] = None, current_user_email: str = Depends(get_current_user)):
    try:
        mydb = get_mysql_connection()
        cursor = mydb.cursor()
        ...
        # Create chat
        sql_create_chat = "INSERT INTO chats (created_at, updated_at) VALUES (%s, %s)"
        val_create_chat = (created_at, created_at)
        cursor.execute(sql_create_chat, val_create_chat)
        chat_id = cursor.lastrowid

        # Add participant if provided
        if chat_participant_id:
            sql_add_participant = "INSERT INTO chat_participants (chat_id, user_id) VALUES (%s, %s)"
            val_add_participant = (chat_id, chat_participant_id)
            cursor.execute(sql_add_participant, val_add_participant)
...
def add_user_to_chat(chat_id: int, user_id: int, current_user_email: str = Depends(get_current_user)):
    try:
        ...
        val_add_user_to_chat = (chat_id, user_id)
        cursor.execute(sql_add_user_to_chat, val_add_user_to_chat)
