...
def read_users_me(current_user_email: str = Depends(get_current_user)):
    try:
        ...
        # Fetch industry and specialization details
        cursor.execute("SELECT industry, specialization FROM users WHERE email = %s", (current_user_email,))
        industry_specialization = cursor.fetchone()
        user["industry"] = industry_specialization["industry"]
        user["specialization"] = industry_specialization["specialization"]
