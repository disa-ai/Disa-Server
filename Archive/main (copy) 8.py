...
class User(BaseModel):
    ...
    industry: Optional[str] = None  # Optional field for industry
    specialization: Optional[str] = None  # Optional field for specialization
...
# Get current user
# Get current user with industry and specialization details from Template table
# Get current user with industry and specialization details from Template table
...
def read_users_me(current_user_email: str = Depends(get_current_user)):
    try:
        ...
        # Fetch user details including template_id
        cursor.execute("SELECT * FROM users WHERE email = %s", (current_user_email,))
        user = cursor.fetchone()
        if user is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User not found")

        # Fetch industry and specialization details from Template table based on template_id
        template_id = user.get('template_id')
        if template_id is not None:
            cursor.execute("SELECT industry, specialization FROM templates WHERE id = %s", (template_id,))
...
def get_chat_members(chat_id: int, current_user_email: str = Depends(get_current_user)):
    try:
        ...
        # Retrieve members of the chat, excluding the current user
