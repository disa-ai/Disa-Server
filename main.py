...
def create_template(template: TemplateCreate, current_user_email: str = Depends(get_current_user)):
    try:
        mydb = get_mysql_connection()
        cursor = mydb.cursor(dictionary=True)
        sql = """INSERT INTO templates (industry, specialization, template_text, checklist) 
                 VALUES (%s, %s, %s, %s)"""
        values = (template.industry, template.specialization, template.template_text, template.checklist)
...
def update_template(template_id: int, template: TemplateUpdate, current_user_email: str = Depends(get_current_user)):
    try:
        ...
        sql = """UPDATE templates 
                 SET industry = %s, specialization = %s, template_text = %s, checklist = %s
                 WHERE id = %s"""
        values = (template.industry, template.specialization, template.template_text, template.checklist, template_id)
        cursor.execute(sql, values)
        mydb.commit()
